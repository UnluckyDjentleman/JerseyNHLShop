import { Controller, Get, Post, Put, Delete, UseGuards, UseInterceptors, UploadedFile, Body, Param, } from '@nestjs/common';
...
export class JerseyController {
    constructor(private orderService: OrderService, private jerseyService: JerseyService){}
    ...
    addJersey(@Body() dto: JerseyDTO, @UploadedFile() file, @GetUser() user: users){
        console.log(file);
    ...
    @Put(':id')
    @UseInterceptors(FileInterceptor('jerseyImage'))
    updateJerseyInfo(@Body() dto: JerseyDTO, @Param('id') id: number, @UploadedFile() file){
    ...
    deleteJersey(@Param('id') id: number){
        const deleteOrderWithJersey=this.orderService.deleteOrdersByJerseyId(Number(id))
